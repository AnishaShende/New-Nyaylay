                ...
                similarCases.map((caseItem, index) => (
                  <div key={index}>
                    <Card
                      ...
                      date={caseItem?.date || "No date"}
                      description={caseItem?.decision || "No decision available"}
                      link="" // Link should be a route to the visualization page
                      caseno={caseItem?.case_id || "Case ID not available"}
                      court={caseItem?.court || "Unknown Court"}
                      casestatus={caseItem?.case_status || "Unknown status"}
                      judge={caseItem?.judges_involved || "Unknown Judge"}
                      sect={caseItem?.sections_clauses || "No sections or clauses"}
                      facts={caseItem?.facts || "No facts provided"}
                      petition={caseItem?.petition_filed || "No petition filed"}
                      legalissues={caseItem?.legal_issues || "No legal issues provided"}
                      keylegalques={caseItem?.key_legal_questions || "No key legal questions provided"}
                      plaintiffarguments={caseItem?.plaintiff_arguments || "No plaintiff arguments"}
                      defendantarguments={caseItem?.defendant_arguments || "No defendant arguments"}
                      courtsreasoning={caseItem?.court_reasoning || "No court reasoning provided"}
                      decision={caseItem?.decision || "No decision made"}
                      conclusion={caseItem?.conclusion || "No conclusion"}
                      casesummary={caseItem?.case_summary || "No case summary"}
                    >
                      <Link to={`/visualize/${caseItem.case_id}`} className="text-blue-500 underline">
                        Visualize
                      </Link>
                    </Card>
                  </div>
